...
export abstract class RabbitMqQueueExchange {
  ...
  private async withChannel<T>(
    ...
    if (!conn) {
      this.logger.error(
        "Cannot perform operation — no active connection",
        "Exchange",
    ...
    const channel = await ChannelManager.createChannel(conn, () => {
      this.logger.warn("Exchange channel closed", "Exchange", {
        exchange: this.exchangeName,
      });
    });
    try {
  ...
  protected async createExchange() {
    this.logger.info("Asserting exchange", "Exchange", {
      exchange: this.exchangeName,
    ...
    this.logger.info("Exchange asserted", "Exchange", {
      exchange: this.exchangeName,
    });
  }

  protected async deleteExchange(ifUnused = false) {
    this.logger.info("Deleting exchange", "Exchange", {
      exchange: this.exchangeName,
      ifUnused,
    });
    await this.withChannel((ch) =>
  ...
  protected async createQueue(
    ...
    this.logger.info("Asserting queue", "Exchange", {
      queue: queueName,
      bindKey,
      exchange: this.exchangeName,
    });
    ...
    this.logger.info("Queue asserted and bound", "Exchange", {
      queue: queueName,
      bindKey,
    });
    return result;
  ...
  protected async createDeadLetterQueue(
    ...
    this.logger.info("Asserting dead letter queue", "Exchange", {
      queue: queueName,
      dlxExchange,
      dlxRoutingKey,
      ttl,
  ...
  protected async deleteQueue(
    ...
    this.logger.info("Deleting queue", "Exchange", {
      queue: queueName,
      ifEmpty,
      ifUnused,
    });
  ...
  protected async unbindQueue(
    ...
    this.logger.info("Unbinding queue", "Exchange", {
      queue: queueName,
      bindKey,
      exchange: this.exchangeName,
    });
    ...
    this.logger.info("Queue unbound", "Exchange", {
      queue: queueName,
      bindKey,
    });
  }
